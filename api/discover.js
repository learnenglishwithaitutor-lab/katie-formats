  ...
  try {
    ...
    if (action === 'creators' && datasetId) {
      ...
      const creators = Object.values(byAuthor).map(a => {
        ...
        // Videos that beat their own median by 3x or more.
        const outliers = sorted.filter(v => med > 0 && v.views >= med * 3);
      ...
      // Rank: tutorial-style first, then reach per follower, then how big the
      // best outlier was. reachRatio leads because it survives account size.
      creators.sort((a, b) => {
    ...
    // ── Run status (shared by both phases) ─────────────────────────────
