...
async function startRun(input) {
  ...
  if (!data.data?.id) {
    // Apify's own message is the useful part — a quota or billing block reads
    // as a generic failure otherwise, and you go looking in the wrong place.
    const why = data.error?.message || data.message || 'Apify would not start the run';
    return { error: why, apifyType: data.error?.type || null, debug: data };
  }
