...
const CLASSIFY_SCHEMA = {
  ...
  properties: {
    ...
    production: {
      type: 'string',
      enum: ['simple', 'skit', 'unclear'],
      description:
        'simple = one person speaking straight to camera in one ordinary ' +
        'setting. skit = acting or roleplay — the creator plays characters, ' +
        'wears costumes or wigs, appears twice in one frame, acts out a scene ' +
        'with other people, or stages a scenario. unclear = cannot tell.'
    },
    onCameraCount: {
      type: 'integer',
  ...
  required: ['format', 'production', 'onCameraCount', 'note'],
  additionalProperties: false
};

const PROMPT =
  'These are cover frames from one TikTok creator who posts English-teaching ' +
  'videos. Answer two things about them.\n\n' +
  'FORMAT — does this creator film themselves talking to camera?\n' +
  ...
  'PRODUCTION — is it a plain piece to camera, or is the creator acting?\n' +
  'Answer "simple" when it is one person speaking directly to the lens in an ' +
  'ordinary room, car, or street — the kind of video one person could film in ' +
  'one take on a phone.\n' +
  'Answer "skit" when the frames show performance rather than address: the ' +
  'same person appearing twice in one frame or playing multiple roles, wigs, ' +
  'costumes or character props, two or more people acting out an exchange, a ' +
  'staged or dressed set, or a scene being dramatised rather than explained. ' +
  'A second person only matters if they are being acted with — a street ' +
  'interview is still simple.\n' +
  'If the frames only show a person talking and you see no sign of acting, ' +
  'answer "simple" rather than "unclear".';
...
async function classifyOne(creator, framesPerCreator) {
  ...
  if (!urls.length) {
    return { handle: creator.handle, format: 'unclear', production: 'unclear', onCameraCount: 0, note: 'No cover images.' };
  }

  const images = (await Promise.all(urls.map(fetchImageBlock))).filter(Boolean);
  if (!images.length) {
    return { handle: creator.handle, format: 'unclear', production: 'unclear', onCameraCount: 0, note: 'Covers could not be loaded.' };
  }
  ...
  if (data.type === 'error') {
    return { handle: creator.handle, format: 'unclear', production: 'unclear', onCameraCount: 0, note: data.error?.message || 'API error' };
  }
  // A safety decline returns HTTP 200 with stop_reason "refusal" and no usable
  // content — check before reading content[0].
  if (data.stop_reason === 'refusal') {
    return { handle: creator.handle, format: 'unclear', production: 'unclear', onCameraCount: 0, note: 'Classification declined.' };
  }

  const text = (data.content || []).find(b => b.type === 'text')?.text;
  if (!text) {
    return { handle: creator.handle, format: 'unclear', production: 'unclear', onCameraCount: 0, note: 'Empty response.' };
  }
  ...
  try {
    ...
    return { handle: creator.handle, format: 'unclear', production: 'unclear', onCameraCount: 0, note: 'Unparseable response.' };
  }
}
