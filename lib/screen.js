...
const SCREEN_SCHEMA = {
  ...
  properties: {
    ...
    needsPhysicalDemo: {
      type: 'boolean',
      description:
        'True only when the lesson depends on something physically present ' +
        'with the creator — a real object held to camera, mouth or tongue ' +
        'position, a gesture that carries the meaning. ' +
        'Pictures, graphics, text or emoji added on screen do NOT count: ' +
        'those are edited in afterwards and anyone can reproduce them. A ' +
        'vocabulary quiz illustrated with overlaid images needs no physical ' +
        'demo. Ask whether a different person, reading the same script in a ' +
        'plain room, could make this video — if yes, answer false.'
    },
    mentionsBritish: {
      type: 'boolean',
      description:
    ...
    isSelfPromo: {
      type: 'boolean',
      description:
        'True ONLY when the video teaches nothing at all and exists purely to ' +
        'sell — an app, course, ebook, coaching or a sponsor. ' +
        'Almost every creator plugs something at the end; many plug in every ' +
        'single video. That is a habit, not an advert. If a viewer who ' +
        'skipped the last few seconds would still have learnt something, ' +
        'answer false. Strip the plug away, and ask whether a lesson remains.'
    },
    isWhiteboard: {
      type: 'boolean',
      description:
  ...
  required: ['contentType', 'isSkit', 'needsPhysicalDemo', 'mentionsBritish',
             'isSelfPromo', 'isWhiteboard', 'whiteboardConvertible', 'topic',
             'note'],
...
function applyRules(v) {
  ...
  // Belt and braces on the one rule that misfired hardest: a video is only an
  // advert if it also failed to be a lesson. Anything the screener could name
  // a subject for taught something, whatever it was selling at the end.
  if (v.isSelfPromo && (v.contentType === 'other' || v.contentType === 'motivation')) {
    reasons.push('an advert, not a lesson');
  }
