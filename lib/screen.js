...
const SCREEN_SCHEMA = {
  ...
  properties: {
    ...
    isSelfPromo: {
      type: 'boolean',
      description:
        'True when the point of the video is to sell or push something — the ' +
        'creator\'s own app, course, ebook, coaching or a sponsor. A short ' +
        'sign-off plug at the end of a real lesson does not count; the video ' +
        'has to be the advert.'
    },
    isWhiteboard: {
      type: 'boolean',
      description:
...
function applyRules(v) {
  ...
  if (v.isSelfPromo)       reasons.push('an advert, not a lesson');
